...
router = APIRouter(prefix="/userdb",
                    tags=["userdb"],
                    responses={status.HTTP_404_NOT_FOUND : {"menssage": "User not found"}})
...
async def user(user: User):
    if type(search_user("_id", user.email)) == User:
    ...
    # el nombre del campo de la clave unica, es "_id" (no es "id")
